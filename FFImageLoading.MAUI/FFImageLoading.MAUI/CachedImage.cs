...
using FFImageLoading.Forms.MAUI;
...
#region ICachedImage
		...
		private static void OnSourcePropertyChanged(BindableObject bindable, object oldValue, object newValue)
		{
			...
			{
#if ANDROID
				if (ffdroid.FFImageLoading.ImageService.EnableMockImageService)
					return;
#endif
		...
		public static async Task InvalidateCache(ImageSource source, Cache.CacheType cacheType, bool removeSimilar = false)
		{
			...
			if (source is FileImageSource fileImageSource)
				await ffdroid.FFImageLoading.ImageService.Instance.InvalidateCacheEntryAsync(fileImageSource.File, cacheType, removeSimilar).ConfigureAwait(false);

			if (source is UriImageSource uriImageSource)
				await ffdroid.FFImageLoading.ImageService.Instance.InvalidateCacheEntryAsync(uriImageSource.Uri.OriginalString, cacheType, removeSimilar).ConfigureAwait(false);

			if (source is EmbeddedResourceImageSource embResourceSource)
				await ffdroid.FFImageLoading.ImageService.Instance.InvalidateCacheEntryAsync(embResourceSource.Uri.OriginalString, cacheType, removeSimilar).ConfigureAwait(false);
#endif
		}
		...
		{
#if ANDROID
			return ffdroid.FFImageLoading.ImageService.Instance.InvalidateCacheEntryAsync(key, cacheType, removeSimilar);
#else
		...
		protected internal virtual void SetupOnBeforeImageLoading(out Work.TaskParameter imageLoader, IImageSourceBinding source, IImageSourceBinding loadingPlaceholderSource, IImageSourceBinding errorPlaceholderSource)
		{
			...
			{
				imageLoader = ffdroid.FFImageLoading.ImageService.Instance.LoadUrl(source.Path, CacheDuration);
			}
			else if (source.ImageSource == Work.ImageSource.CompiledResource)
			{
				imageLoader = ffdroid.FFImageLoading.ImageService.Instance.LoadCompiledResource(source.Path);
			}
			else if (source.ImageSource == Work.ImageSource.ApplicationBundle)
			{
				imageLoader = ffdroid.FFImageLoading.ImageService.Instance.LoadFileFromApplicationBundle(source.Path);
			}
			else if (source.ImageSource == Work.ImageSource.Filepath)
			{
				imageLoader = ffdroid.FFImageLoading.ImageService.Instance.LoadFile(source.Path);
			}
			else if (source.ImageSource == Work.ImageSource.Stream)
			{
				imageLoader = ffdroid.FFImageLoading.ImageService.Instance.LoadStream(source.Stream);
			}
			else if (source.ImageSource == Work.ImageSource.EmbeddedResource)
			{
				imageLoader = ffdroid.FFImageLoading.ImageService.Instance.LoadEmbeddedResource(source.Path);
			}
			else
#endif
		...
		protected override void OnSizeAllocated(double width, double height)
		{
			...
			if(_reloadBecauseOfMissingSize)
			{
				...
				{
#if ANDROID
					ffdroid.FFImageLoading.ImageService.Instance.Config.Logger?.Error("Couldn't read view size for auto sizing");
#endif
				}
