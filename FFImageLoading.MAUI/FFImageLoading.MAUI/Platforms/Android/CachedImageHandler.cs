...
using FFImageLoading.MAUI.Platforms.Android;
...
namespace FFImageLoading.MAUI
{
    ...
    using ffdroid::FFImageLoading.Extensions;
    ...
    public partial class CachedImageHandler : ViewHandler<ICachedImage, CachedImageView>
    {
        ...
        private void UpdateBitmap(CachedImageView imageView, CachedImage image)
        {
            lock (_updateBitmapLock)
            {
                ...
                if (imageLoader != null)
                {
                    ...
                    if (!_isDisposed)
                        _currentTask = ffdroid.FFImageLoading.TaskParameterPlatformExtensions.Into(imageLoader, imageView);
                }
            }
        }
        ...
        private async void ImageLoadingSizeChanged(CachedImage element, bool isLoading)
        {
            ...
            await ffdroid::FFImageLoading.ImageService.Instance.Config.MainThreadDispatcher.PostAsync(() =>
            {
                if (element == null || _isDisposed)
                    return;
