...
#pragma warning restore RECS0108 // Warns about static fields in generic types
		...
		private async Task<DataResolverResult> Decode(
#if ANDROID
			SKPicture picture, SKBitmap bitmap, DataResolverResult resolvedData
#endif
			)
		{
		...
		public async Task<DataResolverResult> Resolve(string identifier, TaskParameter parameters, CancellationToken token)
		{
			...
			if (UseDipUnits)
			{
				sizeX = ImageService.Instance.DpToPixels(VectorWidth);
				sizeY = ImageService.Instance.DpToPixels(VectorHeight);
			}
